...
public class Header {
	...
	Header(){
		...
		timer = new Timeline(new KeyFrame(Duration.millis(100), e -> {	
			...
			if(isdone) {
				...
				fixborder(field.givelabels(),emptyfields.get(randomvalues.get(isdoneI))/10,emptyfields.get(randomvalues.get(isdoneI))%10,"0,235,0");
		...
		tip.setOnMouseClicked(e->{
			...
			fixborder(field.givelabels(),i/9,i%9,"0,235,0");
			...
			for (int i = 0; i < 81; i++) {
						...
						&&field.givsudoku()[i/9][i%9]==Integer.parseInt(field.givelabels()[i/9][i%9].getText()))
				{
					fixborder(field.givelabels(),i/9,i%9,"0,235,0");
						...
						&&field.givsudoku()[i/9][i%9]!=Integer.parseInt(field.givelabels()[i/9][i%9].getText()))
				{
					fixborder(field.givelabels(),i/9,i%9,"230, 0, 0");
				}			
	...
	private void fixborder(TextField[][] l,int y,int x,String s) {
